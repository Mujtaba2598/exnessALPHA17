...
// ⭐ ALWAYS USE ENVIRONMENT VARIABLE FOR JWT_SECRET – no hardcoded fallback
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) {
  console.error('❌ JWT_SECRET is not set! Please set it in Render environment variables.');
  process.exit(1);
}
...
// ✅ CORRECT: TickerAll API key from environment or hardcoded
...
console.log('📦 Version: 17.0.1 (Fixed Auth)');
console.log('🔑 TICKERALL_API_KEY:', TICKERALL_API_KEY ? '✅ Found' : '❌ Missing');

// ==================== TICKERALL INITIALIZATION ====================
let ticker = null;

if (TICKERALL_API_KEY && TICKERALL_API_KEY !== '') {
    try {
        ticker = new Tickerall({ apiKey: TICKERALL_API_KEY });
        console.log('✅ TickerAll initialized successfully');
    } catch (error) {
        console.error('❌ TickerAll initialization error:', error.message);
    }
} else {
    console.log('⚠️ TICKERALL_API_KEY not set. Please add to Render environment variables.');
}
...
app.get('/api/admin/user-balances', authenticate, async (req, res) => {
    ...
    for (const [email, userData] of Object.entries(users)) {
        ...
        try {
            const accountInfo = await ticker.accounts.get(userData.tickerallSessionId);
            balances[email] = { 
...
// ==================== EXNESS ACCOUNT ROUTES (TickerAll) ====================
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ success: false, message: 'TickerAll not initialized. Please set TICKERALL_API_KEY in environment variables.' });
        }
...
// ==================== AI SIGNAL GENERATION (unchanged) ====================
...
class HalalTradingEngine {
    ...
    async executeTrade(symbol, side, signal) {
        ...
        try {
            const accountInfo = await ticker.accounts.get(this.accountId);
            ...
            // ✅ CORRECT: No 'm' suffix - uses exact symbol name
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ success: false, message: 'TickerAll not initialized. Please set TICKERALL_API_KEY.' });
        }
